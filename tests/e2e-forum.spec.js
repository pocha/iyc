...
test.describe('Forum End-to-End Tests', () => {
  ...
  test('Complete forum workflow: create post, comment, edit, delete', async ({ page }) => {
    ...
    // Create test images
    const testImage1Path = `/tmp/test_e2e_image_1_${timestamp}.png`;
    const testImage2Path = `/tmp/test_e2e_image_2_${timestamp}.png`;
    
    execSync(`convert -size 300x200 xc:blue -pointsize 24 -fill white -gravity center -annotate +0+0 "Test Image 1\\n${timestamp}" "${testImage1Path}"`);
    execSync(`convert -size 300x200 xc:green -pointsize 24 -fill white -gravity center -annotate +0+0 "Test Image 2\\n${timestamp}" "${testImage2Path}"`);
    ...
    try {
      ...
      console.log('Cleanup: Some test images may have already been removed');
    }
  });
});
